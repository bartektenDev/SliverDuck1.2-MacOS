...
Main() {
    ...
    clear
    Echo "******* iOS-DFU Checker *******"
    Echo " - iOS-DFU script by @ios_euphoria - "
    echo
...
#    [[ -n $CurrentVersion ]] && Echo "* Version: $CurrentVersion"
    ...
    if [[ $DeviceState == "Normal" ]];  then
        osascript -e 'tell app "System Events" to display dialog "Device connected in normal mode. \nSwitch device to DFU mode, then try again."'
        ExitWin 0
...
#            EnterPwnDFU
#        fi
        osascript -e 'tell app "System Events" to display dialog "Now switch to arduino PWN!\n\niPad 2,4? Set checkm8-a5.ino file to:\n#define A5_8942\n\niPad 2,1 - iPad 2,2 - iPad 2,3? Set checkm8-a5.ino file to:\n#define A5_8940\n\nPWN the device using the arduino. After the light turns on, switch back to original iPhone cable and press Ok to start downgrade..."'
        Log "Ready to boot ramdisk, the device is in DFU!"
        bash ../euphoria_scripts/fire/bootramdisk1iPad2.sh
