...
GetDeviceValues() {
    ...
    if [[ -n $DeviceState ]]; then
        ...
        osascript -e 'tell app "System Events" to display dialog "Device not detected in DFU! Put the device into DFU mode. Use only the iPhone cable."'
        ExitWin 1
    fi
...
EnterPwnDFU() {
    ...
    if [[ $DeviceProc == 4 ]]; then
        ...
        Log "WARNING FAIL - Failed to detect device in pwnDFU mode."
...
kDFU() {
    ...
    osascript -e 'tell app "System Events" to display dialog "Device connected in normal mode.\n\nPut the device into DFU mode using only the iPhone cable.\n\nOnce the device is in DFU, try again."'
#    PatchiBSS
