...
Main() {
    ...
    clear
    Echo "******* iOS-Detector BASH *******"
    Echo " - Detection script by @ios_euphoria - "
    echo
...
#[[ -n $CurrentVersion ]] && Echo "* Version: $CurrentVersion"
    ...
    if [[ $? != 0 ]]; then
        osascript -e 'tell app "System Events" to display dialog "Connect to Internet!\n\nDo NOT use a proxy."'
...
#        fi
        ...
        SendPwnediBSS
        Log "Downgrading $ProductType in arduino PWNED DFU mode..."
        ...
        if [[ $DeviceProc == 4 || $DeviceProc == 6 ]] && [[ $platform != "win" ]]; then
            ...
            osascript -e 'tell app "System Events" to display dialog "Device in rec mode"'
...
#            RecoveryExit
    ...
    Log "Detect device in normal/recovery script done!"
#    ExitWin 0
}
